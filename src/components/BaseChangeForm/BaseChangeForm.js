...
const createTodo = (todo) => {
  return {
    ...
    title: todo.title,
    status: todo.status ? todo.status : todo.todoStatus.value,
  ...
  const onSubmit = (data) => {
    if (type === buttonTypes.toAdd) {
      ...
      if (todo.title !== data.title || todo.status !== data.todoStatus.value) {
        const updatedTodo = createTodo({ id: todo.id, title: data.title, status: data.todoStatus.value, time: todo.time })
